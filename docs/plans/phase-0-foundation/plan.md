...
3. **Backend scaffold**: `npm init` inside `server/`; install `express`; `src/index.js` + `src/app.js` with a single `GET /health` route returning a hardcoded success shape. Plain JavaScript (CommonJS, `require`/`module.exports`) — no compiler, no type-def dependencies. Verify `npm run dev` (`node --watch src/index.js`) boots and `curl localhost:<PORT>/health` responds.
...
6. **Prisma client singleton**: `server/src/database/prisma.js` exporting one `new PrismaClient()` instance, imported everywhere else (never instantiate a second client).
7. **Env validation**: `server/src/config/env.js` reads and validates `PORT`, `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `CORS_ORIGIN` from `process.env`, throwing/exiting with a clear message if any required var is missing. Wire it to run at the top of `src/index.js`.
8. **Shared error/response infra**:
   - `utils/ApiError.js` — class `ApiError extends Error` with `statusCode`/`errors` set in the constructor.
   - `utils/apiResponse.js` — `sendSuccess(res, data, message, statusCode = 200)`.
   - `middlewares/asyncHandler.js` — wraps `(req,res,next) => Promise` handlers, forwarding rejections to `next`.
   - `middlewares/notFoundHandler.js` — catches unmatched routes, responds with the error envelope, 404.
   - `middlewares/errorHandler.js` — last-registered middleware; if `err instanceof ApiError` use its `statusCode`/`message`/`errors`, else default to 500 + generic message (never leak stack traces in the response body).
   - Wire all of the above into `app.js` in the correct order (routes → `notFoundHandler` → `errorHandler`).
